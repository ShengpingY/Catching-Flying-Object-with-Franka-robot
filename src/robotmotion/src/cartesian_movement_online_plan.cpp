...
int main(int argc, char** argv) {
  ...
  try {
    ...
    state_dest_x.dq = 0.0013134; state_dest_y.dq = 0.0000043534; state_dest_z.dq = 0.00054352346;
    ...
    robot.control([&current_state, &t_count,&secs, &secs_last, &state_last_x,&state_last_y,&state_last_z,&state_current_x, &state_current_y, &state_current_z, &state_dest_x, &state_dest_y, &state_dest_z](const franka::RobotState& robot_state,
    ...
    // ROS_INFO("time counter is %d miliseconds",time_counter);
      ...
      state_current_x = plan.cubic_trajectory_generator(state_current_x, state_dest_x,0.2,1,0.001);
      // state_current_x = plan.OnlinePlanning(state_current_x, state_dest_x, 0.01, 0.001);
      ...
      // state_current_y = plan.OnlinePlanning(      // ROS_INFO("x's planned acceleration is %f",state_current_x.ddq );
      // ROS_INFO("x's planned jerk is %f",state_current_x.dddq );state_current_y, state_dest_y, 0.3, 0.001);
